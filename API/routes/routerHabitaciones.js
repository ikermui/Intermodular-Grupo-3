...
router.post('/getFilter2', async (req, res) => {
    try{
        ...
        if (req.body.baja !== undefined) condiciones.baja = req.body.baja;
        if (req.body.camaExtra !== undefined) condiciones.camaExtra = req.body.camaExtra;
        if (req.body.cuna !== undefined) condiciones.cuna = req.body.cuna;
        const data = await ModelUser.find(condiciones);
        if (data.length === 0) {
            return res.status(404).json({ message: 'Documento no encontrado' });
        }
